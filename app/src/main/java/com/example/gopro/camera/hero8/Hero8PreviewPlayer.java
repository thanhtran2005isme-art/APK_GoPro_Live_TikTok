...
public final class Hero8PreviewPlayer implements AutoCloseable {
    ...
    @Nullable private volatile Surface requestedOutputSurface;
    @Nullable private ExecutorService decoderExecutor;
    private volatile boolean running;

    public Hero8PreviewPlayer(@NonNull SurfaceView surfaceView, @NonNull Listener listener) {
        ...
        holder.addCallback(new SurfaceHolder.Callback() {
            ...
            @Override public void surfaceDestroyed(@NonNull SurfaceHolder holder) {
                if (surface == holder.getSurface()) surface = null;
            }
        });
    }
    ...
    public void requestOutputSurface(@NonNull Surface targetSurface) {
        if (!targetSurface.isValid()) {
            postError("Surface fullscreen chưa sẵn sàng.");
            return;
        }
        surface = targetSurface;
        requestedOutputSurface = targetSurface;
        requestThirtyFpsSurface(targetSurface);
    }
    ...
    private void decodeLoop() {
        ...
        try {
            ...
            if (outputSurface == null) {
                if (running) postError("Surface preview chưa sẵn sàng.");
                return;
            }
            ...
            while (running) {
                Surface requested = requestedOutputSurface;
                if (codec != null && requested != null && requested.isValid() && requested != outputSurface) {
                    codec.setOutputSurface(requested);
                    outputSurface = requested;
                    requestedOutputSurface = null;
                    requestThirtyFpsSurface(requested);
                }
                ...
                if (chunk == null) {
                    if (tsPipe.isStreamEnded()) return;
                    if (codec != null) {
                ...
                for (AccessUnit accessUnit : accessUnits) {
                    ...
                    if (codec == null && sps != null && pps != null && snapshot.width > 0 && snapshot.height > 0) {
                        Surface newestSurface = surface;
                        if (newestSurface != null && newestSurface.isValid()) outputSurface = newestSurface;
                        codec = createDecoder(outputSurface, snapshot.width, snapshot.height, sps, pps);
                        requestedOutputSurface = null;
                        postStatus("MediaCodec " + snapshot.width + "x" + snapshot.height + " sẵn sàng; đang chờ IDR frame…");
                    }
                    if (codec == null) continue;

                    boolean containsIdr = containsNal(accessUnit.data, 5);
                    if (waitingForIdr) {
                        if (!containsIdr) continue;
                        waitingForIdr = false;
                        jitterBuffer.clear();
                        framePacer.reset();
                    ...
                    if (ptsUs < 0L) ptsUs = syntheticPtsUs;
                    syntheticPtsUs = Math.max(syntheticPtsUs + DEFAULT_FRAME_DURATION_US, ptsUs + DEFAULT_FRAME_DURATION_US);
            ...
            if (running) postError("MediaCodec preview lỗi: " + exception.getClass().getSimpleName() + ": " + safeMessage(exception));
        } finally {
            if (codec != null) {
    ...
    private static void requestThirtyFpsSurface(@NonNull Surface outputSurface) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R && outputSurface.isValid()) {
            try { outputSurface.setFrameRate(30.0f, Surface.FRAME_RATE_COMPATIBILITY_FIXED_SOURCE); }
            catch (IllegalStateException ignored) {}
        }
    }

    @NonNull
    private static MediaCodec createDecoder(@NonNull Surface surface, int width, int height, @NonNull byte[] sps, @NonNull byte[] pps) throws IOException {
    ...
    private static void queueAccessUnit(@NonNull MediaCodec codec, @NonNull byte[] data, long ptsUs) {
        int position = 0;
    ...
    private static boolean drainOutput(@NonNull MediaCodec codec, @NonNull FramePacer framePacer) {
        ...
        while (true) {
            ...
            if (outputIndex >= 0) {
                ...
                if (renderTimeNs == FramePacer.DROP_FRAME) codec.releaseOutputBuffer(outputIndex, false);
                else {
    ...
    private static final class FramePacer {
        ...
        void reset() { firstPtsUs = Long.MIN_VALUE; firstRenderNs = 0L; }
        long renderTimeNs(long ptsUs) {
            ...
            if (targetNs < nowNs - MAX_LATE_FRAME_NS) return DROP_FRAME;
    ...
    private static byte[] findNal(@NonNull byte[] data, int targetType) {
        ...
        while (true) {
            ...
            if ((data[range.payloadStart] & 0x1F) == targetType) {
                ...
                normalized[3] = 1;
                System.arraycopy(data, range.payloadStart, normalized, 4, range.end - range.payloadStart);
                return normalized;
            }
            cursor = range.end;
        }
    ...
    private static boolean containsNal(@NonNull byte[] data, int targetType) {
        ...
        while (true) {
            ...
            if ((data[range.payloadStart] & 0x1F) == targetType) return true;
            cursor = range.end;
        }
    }
    ...
    private static NalRange nextNal(@NonNull byte[] data, int from) {
        ...
        if (payloadStart >= data.length) return null;
        int next = findStartCode(data, payloadStart);
        return new NalRange(payloadStart, next >= 0 ? next : data.length);
    }
    ...
    private void postStatus(@NonNull String message) { mainHandler.post(() -> listener.onDecoderStatus(message)); }
    private void postError(@NonNull String message) { mainHandler.post(() -> listener.onDecoderError(message)); }
    @NonNull private static String safeMessage(@NonNull Throwable throwable) {
        String message = throwable.getMessage();
        return message == null || message.isBlank() ? "không có message" : message;
    }
    @Override public synchronized void close() { stopInternal(); }

    private static final class NalRange {
        final int payloadStart;
        final int end;
        NalRange(int payloadStart, int end) { this.payloadStart = payloadStart; this.end = end; }
    }

    private static final class AccessUnit {
        @NonNull final byte[] data;
        final long ptsUs;
        AccessUnit(@NonNull byte[] data, long ptsUs) { this.data = data; this.ptsUs = ptsUs; }
    }
    ...
    private static final class TsH264Demuxer {
        ...
        @NonNull List<AccessUnit> consume(@NonNull byte[] data) {
            List<AccessUnit> output = new ArrayList<>();
            for (int offset = 0; offset + TS_PACKET_SIZE <= data.length; offset += TS_PACKET_SIZE) {
                if ((data[offset] & 0xFF) == 0x47) parsePacket(data, offset, output);
            }
            return output;
        }

        private void parsePacket(@NonNull byte[] packet, int packetOffset, @NonNull List<AccessUnit> output) {
            ...
            if (adaptationFieldControl == 3) payloadOffset += 1 + (packet[payloadOffset] & 0xFF);
            int packetEnd = packetOffset + TS_PACKET_SIZE;
            if (payloadOffset >= packetEnd) return;
            if (pid == 0) parsePat(packet, payloadOffset, packetEnd, payloadUnitStart);
            else if (pid == pmtPid) parsePmt(packet, payloadOffset, packetEnd, payloadUnitStart);
            else if (pid == videoPid) parseVideo(packet, payloadOffset, packetEnd, payloadUnitStart, output);
        }
        ...
        private void parsePat(byte[] data, int offset, int end, boolean payloadUnitStart) {
            ...
            if (section < 0 || section + 8 > end || (data[section] & 0xFF) != 0x00) return;
        ...
        private void parsePmt(byte[] data, int offset, int end, boolean payloadUnitStart) {
            int section = sectionOffset(data, offset, end, payloadUnitStart);
            if (section < 0 || section + 12 > end || (data[section] & 0xFF) != 0x02) return;
            int sectionLength = ((data[section + 1] & 0x0F) << 8) | (data[section + 2] & 0xFF);
            int sectionEnd = Math.min(end, section + 3 + sectionLength);
            ...
            while (cursor + 5 <= entriesEnd) {
                ...
                if (streamType == 0x1B) { videoPid = elementaryPid; return; }
                cursor += 5 + esInfoLength;
            }
        }

        private void parseVideo(byte[] data, int offset, int end, boolean payloadUnitStart, @NonNull List<AccessUnit> output) {
            if (payloadUnitStart) {
                if (currentPes.size() > 0) {
                    output.add(new AccessUnit(currentPes.toByteArray(), currentPtsUs));
                    currentPes.reset();
                }
                if (offset + 9 > end || data[offset] != 0 || data[offset + 1] != 0 || data[offset + 2] != 1) {
                    currentPtsUs = -1L;
                    return;
                }
                int flags = data[offset + 7] & 0xC0;
                int headerDataLength = data[offset + 8] & 0xFF;
                currentPtsUs = (flags == 0x80 || flags == 0xC0) && offset + 14 <= end ? parsePtsUs(data, offset + 9) : -1L;
                int elementaryOffset = offset + 9 + headerDataLength;
                if (elementaryOffset < end) currentPes.write(data, elementaryOffset, end - elementaryOffset);
            } else if (currentPes.size() > 0) currentPes.write(data, offset, end - offset);
        }

        private static int sectionOffset(byte[] data, int offset, int end, boolean payloadUnitStart) {
            if (!payloadUnitStart) return offset;
            if (offset >= end) return -1;
            int section = offset + 1 + (data[offset] & 0xFF);
